...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const changeTeamMemberSelect = document.getElementById('change-team-member-select');
    const changeTeamNewTeam = document.getElementById('change-team-new-team');
    ...
    function loadMembers() {
        ...
        if (changeTeamMemberSelect) changeTeamMemberSelect.innerHTML = '<option value="" disabled selected>名前を選択してください</option>';
        const taskPermissionMemberSelect = document.getElementById('task-permission-member-select');
        if (taskPermissionMemberSelect) taskPermissionMemberSelect.innerHTML = '<option value="" disabled selected>名前を選択してください</option>';
        ...
        fetch('/api/members')
            ...
            .then(members => {
                ...
                members.forEach(m => {
                    ...
                    opt3.value = m.id;
                    opt3.textContent = `${m.name} (${m.team || '共通'})`;
                    if(changeTeamMemberSelect) changeTeamMemberSelect.appendChild(opt3);
                    
                    const taskPermissionMemberSelect = document.getElementById('task-permission-member-select');
                    if (taskPermissionMemberSelect) {
                        const opt4 = document.createElement('option');
                        opt4.value = m.id;
                        opt4.textContent = `${m.name} ${m.canAssignTasks ? '✅[権限あり]' : ''}`;
                        taskPermissionMemberSelect.appendChild(opt4);
                    }
                    ...
                    row.style.cssText = "display: flex; gap: 10px; align-items: center; background: #F9FAFB; padding: 8px 10px; border-radius: 8px; flex-wrap: wrap;";
                    ...
                    row.innerHTML = `
                        ...
                        <select class="bulk-status" style="width: 90px; padding: 6px; border-radius: 4px; border: 1px solid #D1D5DB;">
                            ...
                            <option value="連絡無し" ${m.status==='連絡無し'?'selected':''}>連絡無し</option>
                        ...
                        <select class="bulk-location" style="width: 130px; padding: 6px; border-radius: 4px; border: 1px solid #D1D5DB;" ${(m.status !== '出席') ? 'disabled' : ''}>
                    ...
                    const locationSelect = row.querySelector('.bulk-location');
                    statusSelect.addEventListener('change', () => {
                        if (statusSelect.value === '出席') {
    ...
    const taskPermissionForm = document.getElementById('change-task-permission-form');
    const taskPermissionMemberSelect = document.getElementById('task-permission-member-select');
    const taskPermissionAction = document.getElementById('task-permission-action');
    ...
    addForm.addEventListener('submit', (e) => {
        ...
        const teamEl = document.getElementById('new-member-team');
        const team = teamEl ? teamEl.value : '共通';
        ...
        socket.emit('addMember', { adminPassword, name, team }, (res) => {
            if (res.success) {
                showAlert('成功', `${name} さん（${team}）を追加しました。`, 'success');
    ...
    // 所属チームの変更処理
    if (changeTeamForm) {
        changeTeamForm.addEventListener('submit', (e) => {
            e.preventDefault();
            const adminPassword = adminPasswordInput.value;
            const id = changeTeamMemberSelect.value;
            const newTeam = changeTeamNewTeam.value;
            const name = changeTeamMemberSelect.options[changeTeamMemberSelect.selectedIndex]?.textContent.split(' ')[0];
            ...
            socket.emit('changeTeam', { adminPassword, id, newTeam }, (res) => {
                if (res.success) {
                    showAlert('成功', `${name} さんを ${newTeam} に移動しました。`, 'success');
                    changeTeamMemberSelect.value = '';
                } else {
                    showAlert('エラー', res.message, 'error');
                }
            });
    ...
    // タスク権限の変更処理
    if (taskPermissionForm) {
        taskPermissionForm.addEventListener('submit', (e) => {
            e.preventDefault();
            const adminPassword = adminPasswordInput.value;
            const id = taskPermissionMemberSelect.value;
            const canAssign = document.getElementById('task-permission-action').value === 'true';
            ...
            socket.emit('setTaskPermission', { adminPassword, targetId: id, canAssign }, (res) => {
                if (res.success) {
                    const statusStr = canAssign ? '付与' : '剥奪';
                    showAlert('成功', `${res.memberName} さんのリーダー権限を${statusStr}しました。`, 'success');
                    taskPermissionMemberSelect.value = '';
                } else {
                    showAlert('エラー', res.message, 'error');
                }
            });
    ...
    // 過去の出欠状況の上書き処理
    if (pastStatusSelect) {
        const timeContainer = document.getElementById('past-time-container');
        pastStatusSelect.addEventListener('change', () => {
            if (pastStatusSelect.value === '出席' || pastStatusSelect.value === '遅刻') {
                timeContainer.style.display = 'flex';
                if (pastStatusSelect.value === '出席') {
                    document.getElementById('past-start-time').value = '16:30';
                }
            } else {
    ...
    if (overridePastForm) {
        overridePastForm.addEventListener('submit', (e) => {
            ...
            const startTimeStr = document.getElementById('past-start-time').value;
            const endTimeStr = document.getElementById('past-end-time').value;
            ...
            socket.emit('overridePastAttendance', {
                ...
                targetDate: date,
                newStatus: status,
                customStartTimeStr: startTimeStr,
                customEndTimeStr: endTimeStr
            }, (res) => {
                if (res.success) {
