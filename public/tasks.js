document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('login-btn').addEventListener('click', () => {
        ...
        const m = membersList.find(x => x.id === parseInt(id, 10));
        if (m && m.password === pw) {
            loginError.style.display = 'none';
            currentUser = { ...m, password: pw };
            setupDashboard();
        } else {
    ...
    function renderTasks(allTasks) {
        ...
        displayTasks.forEach(task => {
            ...
            const statusBadge = task.isCompleted ? '<span class="task-badge completed">完了済</span>' : '<span class="task-badge">未完了</span>';
            ...
            let html = `
                <div class="task-header">
                    <div>
                        <strong style="font-size: 1.1rem; display:block; margin-bottom:5px;">${task.title}</strong>
                        <div style="font-size: 0.85rem; color:#6b7280;">
                            作成者: ${assignerInfo ? assignerInfo.name : '不明'} → 宛先: ${assigneeInfo ? assigneeInfo.name : '不明'}
                        </div>
                    </div>
                    ${statusBadge}
                </div>
                ${task.desc ? `<div style="font-size: 0.95rem; margin-top:8px; padding:10px; background:#f9fafb; border-radius:6px;">${task.desc}</div>` : ''}
                <div class="task-actions" style="margin-top:10px; justify-content: flex-end;">
            `;
            
            if (isAssignedToMe && !task.isCompleted) {
                html += `<button class="submit-btn mark-complete-btn" data-id="${task.id}" style="padding: 8px 16px; background:#10b981;">完了にする</button>`;
            }
            if (isAssignedToMe && task.isCompleted) {
                html += `<button class="submit-btn mark-incomplete-btn" data-id="${task.id}" style="padding: 8px 16px; background:#6b7280;">未完了に戻す</button>`;
            }
            if (isLeader && task.assignerId === currentUser.id) {
                html += `<button class="submit-btn delete-task-btn" data-id="${task.id}" style="padding: 8px 16px; background:#dc2626;">削除</button>`;
            }
