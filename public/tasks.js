document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTasks(allTasks) {
        const taskListTeam = document.querySelector('#task-list-team .category-grid');
        ...
        if (taskListTeam) taskListTeam.innerHTML = '';
        ...
        if (myTasks.length === 0) {
            ...
            document.getElementById('task-list-team').style.display = 'none';
            ...
            document.getElementById('task-list-team').style.display = 'block';
            ...
            if (tasksIAssigned.length === 0) {
                ...
                assignedTaskListContainer.innerHTML = `
                    <div class="task-category-section" style="margin-bottom:20px;">
                        <h3 style="font-size: 1.1rem; color: #4F46E5; margin-bottom: 10px;">📋 各チームのタスク</h3>
                        <div id="assigned-team" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 15px;"></div>
                    </div>
                    <div class="task-category-section" style="margin-bottom:20px;">
        ...
        const routeTask = (task, isAssignedByMe) => {
            let targetGrid;
            if (isAssignedByMe) {
                if (!document.getElementById('assigned-team')) return; // No tasks assigned
                if (task.category === '各チームのタスク') targetGrid = document.getElementById('assigned-team');
                ...
                else targetGrid = document.getElementById('assigned-team'); // Fallback
            } else {
                if (task.category === '各チームのタスク') targetGrid = taskListTeam;
                else if (task.category === '部全体のタスク') targetGrid = taskListOverall;
                else if (task.category === 'マネージャーのタスク') targetGrid = taskListManager;
                else targetGrid = taskListTeam; // Fallback
            }
        ...
        ['task-list-team', 'task-list-overall', 'task-list-manager'].forEach(id => {
        ...
        if (isLeader && document.getElementById('assigned-team')) {
            [{id: 'assigned-team'}, {id: 'assigned-overall'}, {id: 'assigned-manager'}].forEach(info => {
