...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function appendTaskToContainer(task, container) {
        ...
        let html = `
            <div style="position: absolute; top: 10px; right: 10px;">
                <input type="checkbox" class="bulk-delete-cb" value="${task.id}" style="transform: scale(1.4); cursor: pointer;">
            </div>
            <div class="member-name" style="font-size: 1.1rem; font-weight: bold; margin-bottom: 8px; word-break: break-all; padding-right: 25px;">${task.title}</div>
            <div class="member-status" style="font-size: 0.9rem; padding: 4px; background-color: ${borderColor}; color: white; border: none;">宛先: ${assigneeInfo ? assigneeInfo.name : '不明'}</div>
            
            ${deadlineHtml}
            
            <div style="font-size: 0.85rem; margin-top: 10px; color: #6b7280; text-align: center;">${progressText}</div>
            
            <div style="width: 100%; background: #E5E7EB; border-radius: 4px; overflow: hidden; height: 6px; margin-top: 6px;">
    ...
    if (bulkDeleteBtn) {
        bulkDeleteBtn.addEventListener('click', () => {
            ...
            if(confirm(`選択した${taskIds.length}件のタスクを一括削除しますか？`)){
                const pwd = prompt('管理用パスワードを入力してください:');
                if (pwd) {
                    socket.emit('deleteTasks', { taskIds, password: pwd, memberId: selectedMember.id }, (res) => {
                        if (!res.success) alert('エラー: ' + res.message);
                        else {
                            // Uncheck all manually if needed
