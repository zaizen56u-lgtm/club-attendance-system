document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCalendar() {
            ...
            if (!record.attended) {
                ...
                if (record.lastLeaveTime) {
                    ...
                    if (record.date === todayYMD) {
                        ...
                        timeText += ` 〜 (退席忘れ)`;
                        endIso = null; // nullにすることでカレンダー上で複数日にまたがるのを防ぐ
                    }
                }
        ...
        if (allLogs.length > 0) {
            ...
            const titlePrefix = selectedUser ? '' : `[${selectedUser}] `;

            while (currentDate < today) { // 過去の日付のみ対象
                ...
                // 記録がない（または出席も欠席連絡もない）場合
                if (!record || (!record.attended && !record.excusedAbsent)) {
                    ...
                    const isRestDay = isHoliday || (dayOfWeek === 0 && !isExplicitWeekday && !customTimeObj);
                    
                    if (!isRestDay && !isOffDate) {
                        // 活動日なのに記録がない = 無断欠席
                        // ただし、すでに events に "出席記録なし" などで追加されていないか確認（通常はないはずですが念のため）
                        const alreadyAdded = events.some(e => e.start === yyyyMmDd);
                        if (!alreadyAdded) {
                            events.push({
                                title: `${titlePrefix}無断欠席`,
                                start: yyyyMmDd,
                                color: '#EF4444', // 赤色
                                allDay: true
                            });
