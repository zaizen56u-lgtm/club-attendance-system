...
function updateTrains() {
    ...
    function renderSchedule(containerId, scheduleArray) {
        ...
        if (window.innerWidth > 1000) {
            ...
            maxItems = Math.max(1, Math.floor((window.innerHeight - 350) / 100));
        } else {
            // スマホ版：縦に3列(JR、オレンジ、産交)が並ぶため、余白を引いた高さを3等分する
            const availableHeight = window.innerHeight - 350; // タイトルや時計、余白の合計
            maxItems = Math.max(1, Math.floor((availableHeight / 3) / 100));
        }
