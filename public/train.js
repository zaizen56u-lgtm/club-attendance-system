...
function updateTrains() {
    ...
    function renderSchedule(containerId, scheduleArray) {
        ...
        if (nextTrains.length === 0) {
            const emptyHtml = '<div class="no-train">本日の運行は終了しました</div>';
        ...
        if (window.innerWidth > 1000) {
            // デスクトップ版：横並びなので1列の高さをフルに使える
            // タイトルや時計などの固定ヘッダーで約450px、1列車あたりの高さは約150pxを使用
            maxItems = Math.max(1, Math.floor((window.innerHeight - 450) / 150));
        } else {
            // スマホ版：縦に3列（JR・おれんじ・産交）が並ぶため、高さを3等分する
            // ページタイトルや時計などで約180px、各カードのヘッダーでさらに約150px。合計で約330pxを引く
            const availableHeight = window.innerHeight - 330;
            // スマホ版は最大3本までに制限する
            maxItems = Math.min(3, Math.max(1, Math.floor((availableHeight / 3) / 85)));
        }
        ...
        for (let i = 0; i < nextTrains.length; i++) {
            ...
            // スマホ版の場合、3時間（180分）を超える便は表示しない
            if (window.innerWidth <= 1000 && diffMins > 180) {
                break; // 時刻順なので以降の便も全て3時間以上先
            }
            ...
            if (containerId === 'hisatsu-line-trains') {
                ...
                for (let j = scheduleArray.length - 1; j >= 0; j--) {
                    if (scheduleArray[j].id === train.id) {
                        if (scheduleArray[j].time === train.timeStr) {
                            isLast = true;
                        }
                        break;
                    }
            ...
            const statusHtml = status ? `<div class="status-container" style="font-size: 1.4rem; font-weight: bold; color: ${status.color || 'inherit'}; margin-top: 8px; text-align: right;">${status.text}</div>` : '';
        ...
        // 3時間以内の便がないが、本日中に便が存在する場合のメッセージ
        if (displayedCount === 0 && nextTrains.length > 0) {
            newHtml = `<div class="no-train" style="font-size: 0.9rem; padding: 10px;">3時間以内の便はありません<br><span style="font-size: 0.8rem; color: #9CA3AF;">(次便: ${nextTrains[0].timeStr} 発)</span></div>`;
        }
