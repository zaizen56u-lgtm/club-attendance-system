...
io.on('connection', (socket) => {
  ...
  // 長期休暇の登録処理
  socket.on('submitLongTermLeave', (data, callback) => {
    const { id, startDate, endDate, reason } = data;
    const member = members.find(m => m.id === parseInt(id, 10));
    if (!member) {
        if(callback) callback({ success: false, message: "メンバーが見つかりません" });
        return;
    ...
    while (currentDate <= end) {
        ...
        // 欠席ログを作成
        // カレンダー表示用に、その日の朝9時に休みの連絡を入れたようなタイムスタンプにする
    ...
    // もし指定期間に「本日」が含まれていれば、現在のライブ状況も欠席に更新する
    if (includesToday) {
        member.status = '欠席';
        member.location = '-';
        member.reason = reason || "長期休暇";
        member.attendTime = "-";
        member.leaveTime = "-";
...
function checkMidnightReset() {
    ...
    if (currentDate !== systemSettings.lastResetDate) {
        ...
        // 全メンバーのステータスを初期化または事前登録済みの休みに更新
        members.forEach(member => {
            // 本日付けの「欠席」のログ（※長期休暇などにより事前に登録されたもの）があるかチェック
            const preRegisteredLog = historyLogs.find(log => 
                log.name === member.name && 
                log.status === '欠席' && 
                log.timestamp.startsWith(currentDate) // UTCなので9時間のズレがあるが、YYYY-MM-DDはJSTベースでのISO文字列を生成時に作っているなら一致可能
            );

            // UTCでの保存日時からJSTでのYYYY-MM-DDを取得してチェックする厳密な方法
            const todayAbsenceLog = historyLogs.find(log => {
                if (log.name !== member.name || log.status !== '欠席') return false;
            ...
            if (todayAbsenceLog) {
                // 事前に欠席ログがあった場合は、未設定ではなく欠席にセットする
                if (member.status !== "欠席" || member.reason !== todayAbsenceLog.reason) {
                    member.status = "欠席";
                    member.reason = todayAbsenceLog.reason || "長期休暇";
                    member.attendTime = "-";
                    member.leaveTime = "-";
                    member.location = "-";
                    requiresReset = true;
                }
            } else if (member.status !== "未設定") {
                member.status = "未設定";
                member.reason = "";
                member.attendTime = "-";
                member.leaveTime = "-";
