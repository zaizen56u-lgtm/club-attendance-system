...
io.on('connection', (socket) => {
  ...
  socket.on('addTask', (data, callback) => {
    const { assignerId, assigneeId, assigneeIds, category, title, desc, password, deadline } = data;
    ...
    // Support both single assigneeId (legacy) and array of assigneeIds
    let idsToAssign = [];
    if (assigneeIds && Array.isArray(assigneeIds)) {
        idsToAssign = assigneeIds.map(id => parseInt(id, 10));
    } else if (assigneeId) {
        idsToAssign = [parseInt(assigneeId, 10)];
    }

    if (idsToAssign.length === 0) {
      if(callback) callback({ success: false, message: '対象メンバーが指定されていません' });
      return;
    }
    ...
    if(callback) callback({ success: true, tasks: newTasks });
    saveData();
  });

  socket.on('updateTaskStatus', (data, callback) => {
    const { taskId, isCompleted, progress, memberId, password } = data;
  ...
  socket.on('deleteTasks', (data, callback) => {
    const { taskIds, memberId, password } = data;
    const member = members.find(m => m.id === parseInt(memberId, 10));
    if (!member) {
      if(callback) callback({success: false, message: 'メンバーが見つかりません'});
      return;
    ...
    if (!Array.isArray(taskIds)) {
      if(callback) callback({success: false, message: '無効なデータ形式です'});
      return;
    }
  ...
  // QRコードスキャンによる出欠切り替え
  socket.on('scanQR', (data, callback) => {
    const { id } = data;
    
    // 【連打・過負荷防止】同一クライアントから1秒以内の連続送信はブロック
    const nowTime = Date.now();
    if (clientLastUpdate[socket.id] && nowTime - clientLastUpdate[socket.id] < 1000) {
        if(callback) callback({ success: false, message: "通信が連続しています。少し待ってから再度お試しください。" });
        return;
    ...
    // 現在のステータスから新しいステータスを決定
    let newStatus = "出席";
    let newLocation = "体育館"; // デフォルトの場所
    if (member.status === "出席" || member.status === "遅刻") {
        newStatus = "退席";
        newLocation = "-";
    }
  ...
  socket.on('overridePastAttendance', (data, callback) => {
    const { password, memberId, targetDate, newStatus, customStartTimeStr, customEndTimeStr, customTimeStr } = data;
    
    // デバッグログ出力
    const debugLogStr = `[${new Date().toISOString()}] OVERRIDE REQ: memberId=${memberId}, date=${targetDate}, status=${newStatus}, start=${customStartTimeStr}, end=${customEndTimeStr}, oldTimeStr=${customTimeStr}\n`;
    try { fs.appendFileSync('override_debug.log', debugLogStr); } catch(e){}
    ...
    try {
        ...
        if (customTimeObj) {
            defaultStart = (customTimeObj.start.split(':').length === 2) ? customTimeObj.start + ":00" : customTimeObj.start;
            defaultEnd = (customTimeObj.end.split(':').length === 2) ? customTimeObj.end + ":00" : customTimeObj.end;
        } else if (isHoliday || (dayOfWeek === 6 && !isWeekdayDate) || (dayOfWeek === 0 && !isWeekdayDate)) {
            defaultStart = "09:30:00";
        }

        const effectiveStartTime = customStartTimeStr || customTimeStr;
        
        // 時刻文字列を安全に HH:MM:00 形式に変換するヘルパー
        ...
        const finalStart = formatTimeSafe(effectiveStartTime) || defaultStart;
        const finalEnd = formatTimeSafe(customEndTimeStr) || defaultEnd;

        const tStartISO = new Date(`${targetDate}T${finalStart}+09:00`).toISOString();
        const tEndISO = new Date(`${targetDate}T${finalEnd}+09:00`).toISOString();

        if (newStatus === "出席" || newStatus === "遅刻") {
            historyLogs.push({ memberId: member.id, name: member.name, status: newStatus, location: "-", reason: "管理者修正", timestamp: tStartISO });
    ...
    // もし指定された日付が「今日」であれば、現在のステータスボードの表示も更新する
    const nowJst = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Tokyo' }));
    const todayStr = `${nowJst.getFullYear()}-${String(nowJst.getMonth() + 1).padStart(2, '0')}-${String(nowJst.getDate()).padStart(2, '0')}`;
    
    if (targetDate === todayStr) {
        const timeStr = `${String(nowJst.getHours()).padStart(2, '0')}:${String(nowJst.getMinutes()).padStart(2, '0')}`;
        member.lastUpdate = timeStr;
        member.reason = "管理者修正";
        member.location = "-";
        
        if (newStatus === "出席" || newStatus === "遅刻") {
            // 基本的に修正後の状態を「退席済み」とするか、「出席中」とするか
            // カレンダーには退席時間も入力されているため、ボード上もその時間を入れる
            member.status = newStatus; // ボード上は出席/遅刻にするか、あるいは退席にするか。一旦入力された状態にする
            member.attendTime = customStartTimeStr || defaultStart.slice(0, 5);
            member.leaveTime = customEndTimeStr || defaultEnd.slice(0, 5);
        } else if (newStatus === "欠席") {
            member.status = "欠席";
            member.attendTime = "-";
            member.leaveTime = "-";
      ...
      const addedLogs = historyLogs.filter(log => log.memberId === member.id && log.timestamp.includes(targetDate.split('-')[0]));
      try { fs.appendFileSync('override_debug.log', `[${new Date().toISOString()}] OVERRIDE SUCCESS: before=${beforeCount}, after=${afterCount}, addedLogs=${JSON.stringify(addedLogs)}\n`); } catch(e){}
      ...
      console.error("overridePastAttendance error:", err);
      try { fs.appendFileSync('override_debug.log', `[${new Date().toISOString()}] OVERRIDE ERROR: ${err.message}\n${err.stack}\n`); } catch(e){}
      if(callback) callback({ success: false, message: "時刻の形式またはデータが不正です: " + err.message });
    }
  });
...
function checkMidnightReset() {
    ...
    if (currentDate !== systemSettings.lastResetDate) {
        const previousDateStr = systemSettings.lastResetDate;
        systemSettings.lastResetDate = currentDate;
        ...
        members.forEach(member => {
            // 前日の終了時点で「出席」または「遅刻」のままだった場合、退席忘れとして前日の18:30に退席ログを追加
            if (member.status === "出席" || member.status === "遅刻") {
                try {
                    let checkoutTimeStr = "18:30:00";
                    const customTimeObj = systemSettings.customDates && systemSettings.customDates[previousDateStr];
                    if (customTimeObj && customTimeObj.end) {
                        checkoutTimeStr = (customTimeObj.end.split(':').length === 2) ? customTimeObj.end + ":00" : customTimeObj.end;
                    }
                    const checkoutISO = new Date(`${previousDateStr}T${checkoutTimeStr}+09:00`).toISOString();
                    historyLogs.push({
                        memberId: member.id,
                        name: member.name,
                        status: "退席",
        ...
        if (requiresReset || historyModified) {
            // パスワードを除外して配信
            const publicMembers = members.map(({ password, ...rest }) => rest);
            io.emit('initData', publicMembers); // クライアント全体の表示をリフレッシュ
            saveData();
            console.log(`[${currentJstStr}] 深夜0時の自動リセット・自動退席を実行しました`);
        } else {
...
const matchModule = require('./subsystems/match/match_module.js');
app.use('/subsystems/match/public', express.static(path.join(__dirname, 'subsystems/match/public')));
app.use(matchModule.router);
matchModule.initMatchSystem();
// --------------------------------
