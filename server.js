...
// === 深夜0時の自動リセット機能 ===
// 日本時間(JST)で日付が変わったかを1分ごとにチェックする
let lastCheckedDay = new Date().toLocaleString('en-US', { timeZone: 'Asia/Tokyo', day: 'numeric' });
setInterval(() => {
    const currentJstStr = new Date().toLocaleString('en-US', { timeZone: 'Asia/Tokyo' });
    const currentJst = new Date(currentJstStr);
    const currentDay = currentJst.getDate().toString();
