...
function formatTeamName(name) {
    if (!name) return name;
    if (window.innerWidth <= 800 || window.matchMedia("(orientation: portrait)").matches) {
        return name.replace(/工業高等専門学校/g, '高専')
                   .replace(/高等専門学校/g, '高専')
                   .replace(/チーム/g, '')
                   .trim();
    }
...
let lastSettingFinishId = 0;
let countdownAudioEl = new Audio('custom_buzzer.mp3');
let beepOffset = 0;
let buzzerDuration = 3000;
...
function playSettingFinishBeep() {
    if (!beepAudioCtx) return;
    try {
        const osc = beepAudioCtx.createOscillator();
        const gain = beepAudioCtx.createGain();
        osc.type = 'square';
        osc.frequency.setValueAtTime(880, beepAudioCtx.currentTime);
        gain.gain.setValueAtTime(0.5, beepAudioCtx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.01, beepAudioCtx.currentTime + 1.5);
        osc.connect(gain);
        gain.connect(beepAudioCtx.destination);
        osc.start();
        osc.stop(beepAudioCtx.currentTime + 1.5);
    } catch(e) { console.error("Beep error:", e); }
}

// 自動的に無音部分をカットし、音声の長さも取得するロジック
fetch('custom_buzzer.mp3')
    ...
    .then(audioBuf => {
        ...
        buzzerDuration = (audioBuf.duration - beepOffset) * 1000;
    }).catch(e => console.error("自動カット処理エラー:", e));

function handleStartClick() {
    initAudioContext();
    triggerCountdownSound();
    sendAction('COUNTDOWN_START', { delay: Math.round(buzzerDuration) });
}

function handleSettingClick() {
    initAudioContext();
    sendAction('START_SETTING_TIME');
}

function triggerCountdownSound() {
    countdownAudioEl.currentTime = beepOffset;
    countdownAudioEl.volume = 1.0;
...
async function fetchState() {
    try {
        ...
        const displayTime = state.isSettingTime ? state.settingTimeLeft : state.elapsedTime;
        document.getElementById('op-timer-display').innerText = formatTime(displayTime);
        
        const settingLabel = document.getElementById('op-setting-time-label');
        if (settingLabel) {
            settingLabel.style.display = state.isSettingTime ? 'block' : 'none';
        }
        ...
        if (JSON.stringify(currentTeamList) !== JSON.stringify(state.teamList)) {
            ...
            const optionsHtml = currentTeamList.map(t => `<option value="${t}">${formatTeamName(t)}</option>`).join('');
        ...
        if (state.points && state.itemNames) {
            ...
            if (JSON.stringify(currentKeys.sort()) !== JSON.stringify(newKeys.sort())) {
                ...
                newKeys.forEach(k => {
                    ...
                    let dispName = state.itemNames[k];
                    let nameStyle = "font-weight: bold;";
                    let descStyle = "font-size: 1rem; color:#aaa; margin-top: 4px;";
                    
                    // PC・スマホ共通で名称を短縮
                    if (dispName.includes("全ての得点スポットに得点")) {
                        dispName = "全スポットに得点";
                    }
                    
                    if (window.innerWidth <= 800) {
                        nameStyle += " word-break: keep-all;";
                        descStyle = "font-size: 0.85rem; color:#aaa; margin-top: 4px; white-space: nowrap; letter-spacing: -0.5px;";
                        if (dispName === "全スポットに得点") {
                            nameStyle += " white-space: nowrap; letter-spacing: -0.5px; font-size: 0.95rem;";
                        }
                    } else {
                        // PC表示時でもこの項目は絶対に改行させない
                        if (dispName === "全スポットに得点") {
                            nameStyle += " white-space: nowrap;";
                        }
                    }
                    
                    if (isSpecial) {
                        tdCenter.innerHTML = `<div id="disp-name-${k}" style="${nameStyle}">${dispName}</div><div style="${descStyle}"><span id="disp-lim-${k}">${state.limits[k]}</span>回きり: <span id="disp-pt-${k}">${state.points[k]}</span>点</div>`;
                    } else {
                        tdCenter.innerHTML = `<div id="disp-name-${k}" style="${nameStyle}">${dispName}</div><div style="${descStyle}">1回: <span id="disp-pt-${k}">${state.points[k]}</span>点</div>`;
                    }
        ...
        // カウントダウンサウンドの再生 (iPad側は手動クリック時に直接鳴らすため、ここでは状態追従のみ行う)
        if (state.countdownTriggerId !== undefined) {
            lastCountdownId = state.countdownTriggerId;
        }
        
        // セッティングタイム終了音の再生
        if (state.settingFinishTriggerId !== undefined) {
            if (lastSettingFinishId === 0 && state.settingFinishTriggerId > 0) {
                lastSettingFinishId = state.settingFinishTriggerId;
            } else if (state.settingFinishTriggerId !== lastSettingFinishId) {
                lastSettingFinishId = state.settingFinishTriggerId;
                playSettingFinishBeep();
            }
        }
