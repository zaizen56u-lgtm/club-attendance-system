            ...
            if (JSON.stringify(currentKeys.sort()) !== JSON.stringify(newKeys.sort())) {
                ...
                newKeys.forEach(k => {
                    ...
                    if (window.innerWidth <= 800) {
                        dispName = dispName.replace("全ての得点スポットに得点という物", "全得点スポット<wbr>に得点");
                    }
                    
                    if (isSpecial) {
                        tdCenter.innerHTML = `<div id="disp-name-${k}" style="font-weight: bold; word-break: keep-all;">${dispName}</div><div style="font-size: 0.85rem; color:#aaa; margin-top: 4px; white-space: nowrap; letter-spacing: -0.5px;"><span id="disp-lim-${k}">${state.limits[k]}</span>回きり: <span id="disp-pt-${k}">${state.points[k]}</span>点</div>`;
                    } else {
                        tdCenter.innerHTML = `<div id="disp-name-${k}" style="font-weight: bold; word-break: keep-all;">${dispName}</div><div style="font-size: 0.85rem; color:#aaa; margin-top: 4px; white-space: nowrap;">1回: <span id="disp-pt-${k}">${state.points[k]}</span>点</div>`;
                    }
