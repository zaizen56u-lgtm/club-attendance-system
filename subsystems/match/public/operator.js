        ...
        if (state.points && state.itemNames) {
            ...
            if (JSON.stringify(currentKeys.sort()) !== JSON.stringify(newKeys.sort())) {
                ...
                newKeys.forEach(k => {
                    ...
                    if (isSpecial) {
                        tdCenter.innerHTML = `<div id="disp-name-${k}" style="font-weight: bold;">${state.itemNames[k]}</div><div style="font-size: 0.85rem; color:#aaa; margin-top: 4px; white-space: nowrap; letter-spacing: -0.5px;"><span id="disp-lim-${k}">${state.limits[k]}</span>回きり: <span id="disp-pt-${k}">${state.points[k]}</span>点</div>`;
                    } else {
                        tdCenter.innerHTML = `<div id="disp-name-${k}" style="font-weight: bold;">${state.itemNames[k]}</div><div style="font-size: 0.85rem; color:#aaa; margin-top: 4px; white-space: nowrap;">1回: <span id="disp-pt-${k}">${state.points[k]}</span>点</div>`;
                    }
