...
let matchState = {
    ...
    breakdownRight: { movingBucket: 0, flag: 0, deskDrawer: 0, edgeBucket: 0, centerBucket: 0, allSpots: 0 },
    chatMessages: [], 
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        if (matchState.isSettingTime) {
            if (matchState.settingTimeLeft > 0) {
                matchState.settingTimeLeft--;
            }
            if (matchState.settingTimeLeft <= 0) {
                stopTimer();
                matchState.isSettingTime = false;
                matchState.settingFinishTriggerId++;
            }
        } else {
...
app.post('/api/action', (req, res) => {
    ...
    switch (action) {
        case 'START':
            startTimer();
            break;
        case 'START_SETTING_TIME':
            stopTimer();
            matchState.isSettingTime = true;
            matchState.settingTimeLeft = 60;
            startTimer();
            break;
        case 'COUNTDOWN_START':
            ...
            const delay = (payload && payload.delay) ? payload.delay : 3000;
            setTimeout(() => {
            ...
            break;
        case 'SAVE_RESULTS': {
            const resultsDir = path.join(__dirname, '../../../match_results');
...
// Start server
app.listen(PORT, () => {
    console.log(`[Match System] Server is running on http://localhost:${PORT}`);
